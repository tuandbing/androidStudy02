    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1 && resultCode == RESULT_OK) {
            if (data != null) {
                ...
                Log.d("TAG", "Received Position: " + position); // 打印 position

                if (position != -1 ) {
                    student = (Student) data.getSerializableExtra("update-student");
                    studentList.set(position, student);
                    Log.d("TAG", "Updated student at position: " + position);
                } else  {
                    student = (Student) data.getSerializableExtra("student");
                    studentList.add(student);
                    Log.d("TAG", "Added new student");
                }

                // 打印当前 studentList 的大小
                Log.d("TAG", "Current studentList size: " + studentList.size());
